...
## Overview
**DrawVault** is a production Document Management System (DMS) built for **Unique Properties**.
It manages construction drawings, transmittals, folder structures, revision history, and document
control workflows across multiple projects.

**Live URLs**
- Frontend: https://drawing-management-delta.vercel.app (Vercel — auto-deploys from `main`)
- Backend:  Railway (auto-deploys from `main` when connected)
...
> Frontend runs on **port 5174** (not 5173 — Vite auto-incremented).
> Backend runs on **port 3000**.

Set `VITE_API_URL=http://localhost:3000` in `dms-frontend/.env` for local development.
...
|----------|----------|------|
| `director` | `Unique123!` | Director |

Additional users are created by the Director via Settings → User Management.
