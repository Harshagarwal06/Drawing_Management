...
export default function ConfirmDeleteModal({ drawing, onConfirm, onCancel, loading }) {
  const { handleBackdrop, panelRef } = useModalClose(onCancel);
        ...
        <div className="p-6">
          ...
          <div className="flex gap-2 justify-end">
            <button
              onClick={onCancel}
              disabled={loading}
              className="min-h-11 px-4 py-2 rounded-lg border border-border-slate bg-white text-on-surface-variant hover:bg-surface-container text-[13px] font-medium transition-colors disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              onClick={onConfirm}
              disabled={loading}
              className="min-h-11 px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 text-[13px] font-medium transition-colors disabled:opacity-60 flex items-center gap-1.5"
            >
              {loading
